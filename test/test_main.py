...
class BVTKBaseTest(unittest.TestCase):
    ...
    def runTestCase(self, blend_file, python_script=None, python_params=None):
        ...
        self.assertTrue(
            ...
            "The provided python script %s was not found" % (python_script),
        )
        ...
        proc = subprocess.Popen(
            [
                self.blender_path,
                "--addons",
                "BVtkNodes",  # Activate the addon, even if not set using the user preferences
                "--background",
                self.blender_files_dir + blend_file,
                "--python-exit-code",
                "1",  # Exit with failure if the python script fails
                "--python",
                python_script,
            ]
        ...
        stdout_msg = proc_output.decode(
            "utf-8"
        )  # Stdout and Stderr will be displayed if the test fails
        stderr_msg = proc_err.decode("utf-8")
        self.assertTrue(
            returncode == 0,
            "Blender STDOUT:%s%s--------------%sSTDERR:%s%s--------------"
    ...
    def compareWrittenFile(self, produced_file, reference_file, pv_type=None):
        if not pv_enabled:
            self.skipTest(
                "Import of pyvista failed. File comparison will not be performed"
            )

        self.assertFalse(
            produced_file.endswith("vtk") and pv_type is None, "Unknown vtk type"
        )

        if pv_type is None:
            file_ending_map = {
                "vtp": pv.PolyData,
                "vtu": pv.UnstructuredGrid,
                "vti": pv.UniformGrid,
            }
        ...
        self.assertTrue(
            np.allclose(output_mesh.points, ref_mesh.points, atol=1e-4),
            "The points of the two meshes differ",
        )
...
class BVTKMainExamples(BVTKBaseTest):
    ...
    def test_append(self):
        self.runTestCase(
            "test_template.blend",
            python_params=["-j", "//../json_files/test_append_triangle.json"],
        )

    def test_clip(self):
        self.runTestCase(
            "test_template.blend",
            python_params=["-j", "//../json_files/test_clip.json"],
        )

    def test_cone(self):
        self.runTestCase(
            "test_template.blend",
            python_params=["-j", "//../json_files/test_cone.json"],
        )

    def test_fin(self):
        self.runTestCase(
            "test_template.blend", python_params=["-j", "//../json_files/test_fin.json"]
        )

    def test_head(self):
        self.runTestCase(
            "test_template.blend",
            python_params=["-j", "//../json_files/test_head.json"],
        )

    def test_kitchen(self):
        self.runTestCase(
            "test_template.blend",
            python_params=["-j", "//../json_files/test_kitchen.json"],
        )
    ...
    def test_glyphs_and_writers(self):
        self.runTestCase(
            "test_template.blend",
            python_params=["-j", "//../json_files/test_glyphs_and_writers.json"],
        )
        self.compareWrittenFile(
            "test_glyphs_and_writers.vtp", "test_glyphs_and_writers.vtp"
        )

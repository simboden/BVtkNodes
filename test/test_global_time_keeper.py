...
def check_cone_validity(cone_node, cone_mesh):
    ...
    assert_quit(
        np.isclose(np.linalg.norm(circle_center - vertices[0]), height, atol=1e-4),
        "The cone had not the expected height of %f" % (height),
    )
    assert_quit(
        np.allclose(
            np.linalg.norm(vertices[1:] - circle_center[np.newaxis], axis=-1),
            radius,
            atol=1e-4,
        ),
...
def test_frame_updates():
    ...
    keyframes = np.concatenate(
        [np.linspace(0.5, 0.5, num=4), np.linspace(0.5, 2.5, num=6)]
    )
    ...
    for frame in range(1, 11):
        ...
        assert_quit(
            np.isclose(cone_node.m_Radius, keyframe),

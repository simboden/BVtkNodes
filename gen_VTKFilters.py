...
class VTKCollisionDetectionFilter(Node, BVTK_Node):
    ...
    m_GenerateScalars     : bpy.props.BoolProperty ( name='GenerateScalars',      default=True )
    m_NumberOfCellsPerNode: bpy.props.IntProperty  ( name='NumberOfCellsPerNode', default=2 )
    m_BoxTolerance        : bpy.props.FloatProperty( name='BoxTolerance',         default=0.0 )
    m_CellTolerance       : bpy.props.FloatProperty( name='CellTolerance',        default=0.0 )
    m_Opacity             : bpy.props.FloatProperty( name='Opacity',              default=1.0 )
    e_CollisionMode       : bpy.props.EnumProperty ( name='CollisionMode',        default="AllContacts", items=e_CollisionMode_items )
    
    b_properties: bpy.props.BoolVectorProperty(name="", size=6, get=BVTK_Node.get_b, set=BVTK_Node.set_b)

    def m_properties( self ):
        return ['m_GenerateScalars','m_NumberOfCellsPerNode','m_BoxTolerance','m_CellTolerance','m_Opacity','e_CollisionMode',]
    def m_connections( self ):
...
class VTKDistancePolyDataFilter(Node, BVTK_Node):
    ...
    m_ComputeCellCenterDistance: bpy.props.BoolProperty( name='ComputeCellCenterDistance', default=True )
    ...
    def m_properties( self ):
        return ['m_ComputeCellCenterDistance','m_ComputeSecondDistance','m_NegateDistance','m_SignedDistance',]
    def m_connections( self ):
        return (['input 0', 'input 1'], ['output 0', 'output 1'], [], []) 
...
class VTKEquirectangularToCubeMapTexture(Node, BVTK_Node):
    ...
    m_CubeMap                     : bpy.props.BoolProperty ( name='CubeMap',                      default=True )
    m_EdgeClamp                   : bpy.props.BoolProperty ( name='EdgeClamp',                    default=True )
    m_Interpolate                 : bpy.props.BoolProperty ( name='Interpolate',                  default=True )
    m_Mipmap                      : bpy.props.BoolProperty ( name='Mipmap',                       default=False )
    m_PremultipliedAlpha          : bpy.props.BoolProperty ( name='PremultipliedAlpha',           default=False )
    m_Repeat                      : bpy.props.BoolProperty ( name='Repeat',                       default=True )
    m_RestrictPowerOf2ImageSmaller: bpy.props.BoolProperty ( name='RestrictPowerOf2ImageSmaller', default=True )
    m_UseSRGBColorSpace           : bpy.props.BoolProperty ( name='UseSRGBColorSpace',            default=False )
    m_BlendingMode                : bpy.props.IntProperty  ( name='BlendingMode',                 default=0 )
    m_CubeMapSize                 : bpy.props.IntProperty  ( name='CubeMapSize',                  default=512 )
    m_IsDepthTexture              : bpy.props.IntProperty  ( name='IsDepthTexture',               default=0 )
    m_TextureType                 : bpy.props.IntProperty  ( name='TextureType',                  default=3553 )
    m_MaximumAnisotropicFiltering : bpy.props.FloatProperty( name='MaximumAnisotropicFiltering',  default=4.0 )
    e_ColorMode                   : bpy.props.EnumProperty ( name='ColorMode',                    default="Default", items=e_ColorMode_items )
    e_Quality                     : bpy.props.EnumProperty ( name='Quality',                      default="Default", items=e_Quality_items )
...
class VTKHausdorffDistancePointSetFilter(Node, BVTK_Node):
    ...
    e_TargetDistanceMethod: bpy.props.EnumProperty( name='TargetDistanceMethod', default="PointToPoint", items=e_TargetDistanceMethod_items )
    
    b_properties: bpy.props.BoolVectorProperty(name="", size=1, get=BVTK_Node.get_b, set=BVTK_Node.set_b)

    def m_properties( self ):
        return ['e_TargetDistanceMethod',]
    def m_connections( self ):
        return (['input 0', 'input 1'], ['output 0', 'output 1'], [], []) 
...
class VTKLagrangianParticleTracker(Node, BVTK_Node):
    ...
    m_GenerateParticlePathsOutput        : bpy.props.BoolProperty ( name='GenerateParticlePathsOutput',         default=True )
    ...
    def m_properties( self ):
        return ['m_AdaptiveStepReintegration','m_GenerateParticlePathsOutput','m_GeneratePolyVertexInteractionOutput','m_CellLengthComputationMode','m_MaximumNumberOfSteps','m_MaximumIntegrationTime','m_StepFactor','m_StepFactorMax','m_StepFactorMin',]
    def m_connections( self ):

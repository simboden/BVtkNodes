...
def get_classes(group):
    ...
    cursor.execute(
        "SELECT id,name,num_in,num_out FROM Class WHERE grp = (SELECT id FROM CGroup WHERE name=? ) ORDER BY name ",
        (group,),
    )
    ...
    for c in classes:
        ...
        cursor.execute(
            "SELECT name,(SELECT name FROM PType WHERE id=Property.type ),size,value,items,banned FROM Property WHERE class = ? ORDER by type,name",
            (c[0],),
        )
...
def generate(group):
    ...
    DIC = {"MENU": group, "CLASSES": [], "BASE": bases[group]}
    ...
    for c in classes:
        ...
        for p in c["props"]:
            ...
            if items:  # is an enum
                ...
                C["ENUM_ITEMS"].append(
                    prefix + name + "_items=[ (x,x,x) for x in " + str(items) + "]"
                )
    ...
    text = (
        "# Generated definitions for VTK class group: "
        + group
        + "\n# VTK version: "
...
filenames = {
    key: (call_dir if len(call_dir) > 0 else ".") + "/../generated_nodes/" + val
